                  ...
                  $flag as xs:boolean, $closetDistance as xs:double, $topK as xs:integer)
{
    let $doc := document
    {  $document/rtree/node }
    let $candidates := <candidates mindist="1000" maxdist="1000" objects="0">
    </candidates>
    return 
...
declare function tk:getLayer_mbr_node_k($document,$root,$mbr,$keywordCollection as xs:string*, 
                           $candidates as node(), $flag as xs:boolean,
                           $closetDistance as xs:double, $topK as xs:integer)
{

if ($document/node) then 
  tk:getLayer_each_node_k($document/node,$root,$mbr,$keywordCollection,$candidates,$flag,$closetDistance,
                          $topK)
else 
   if ($document/leaf) then 
     ...
     tk:getLayer_each_mbr_k($document/mbr,$root,$mbr,$keywordCollection,$candidates,$flag,$closetDistance,
                             $topK)
};

declare function tk:getLayer_each_node_k($document,$root,$mbr, 
                 $keywordCollection as xs:string*, $candidates as node(), 
                 $flag as xs:boolean, $closetDistance as xs:double, $topK as xs:integer)
{  
 if (empty($document)) then 
   ...
   where
     ((if ($flag) 
          then 
             (tk:minDistanceMBR(xs:float($node/@x),xs:float($node/@y),xs:float($node/@z),xs:float($node/@t),
  ...
  (:         (true()) :)
            ...
            (tk:minDistanceMBR(xs:float($node/@x),xs:float($node/@y),xs:float($node/@z),xs:float($node/@t),
              xs:float($mbr/@x),xs:float($mbr/@y),xs:float($mbr/@z),xs:float($mbr/@t),$closetDistance))  
       )     
       
     or (if ($flag) then (tk:getNumberObjectsTopK($candidates) <= $topK) else false())) 
   return         
                 ...
                 $flag as xs:boolean, $closetDistance as xs:double, $topK as xs:integer)
{
   ...
   where 
     ...
     return tk:getLayer_each_leaf_k(tail($document),$root,$mbr,$keywordCollection,
                                tk:getLayer_mbr_node_k($node,$root,$mbr,$keywordCollection,
                                $candidates,$flag,$closetDistance,$topK),
                                $flag,$closetDistance,$topK)
};
     ...
     if ($node/mbr/way) then                                   
           ...
           if ($flag) then
               if ($keywordCollection = " ")
                then tk:getLayer_each_mbr_k(tail($document),$root,$mbr," ",
                                        tk:addOneWayTopK($candidates, $oneway, $topK),$flag,
                                                       $closetDistance,$topK) 
                else tk:getLayer_each_mbr_k(tail($document),$root,$mbr,$keywordCollection,
                                        tk:addKeywordTopK($candidates, $oneway,$keywordCollection,$topK)
                                        ,$flag,$closetDistance,$topK) 
           else
              if ($keywordCollection = " ")
                then tk:getLayer_each_mbr_k(tail($document),$root,$mbr," ",
                                        tk:addOneWayDistance($candidates, $oneway, $closetDistance),
                                        $flag,$closetDistance,$topK) 
                else tk:getLayer_each_mbr_k(tail($document),$root,$mbr,$keywordCollection,
                                      tk:addKeywordDistance($candidates, $oneway,$keywordCollection,
                                      $closetDistance)
                                      ,$flag,$closetDistance,$topK) 
     else  
               ...
               if ($flag) then          
                 ...
                 if ($keywordCollection = " ")
                    then tk:getLayer_each_mbr_k(tail($document),$root,$mbr," ",
                         ...
                         tk:getLayer_each_mbr_k(tail($document),$root,$mbr,$keywordCollection,
                                       tk:addKeywordDistance($candidates, $oneway,$keywordCollection,
                                       $closetDistance)
                                      ,$flag,$closetDistance,$topK) 
};

declare function tk:minDistanceMBR($x,$y,$z,$t,$a,$b,$c,$d, $distanceMBR as xs:double)
{
 let $dist := osm:getDistanceMbr($x,$y,$z,$t,$a,$b,$c,$d)
 return $dist <= $distanceMBR 
};

declare function tk:closestTopKByName($document as node()*, $name as xs:string, $topK as xs:integer)
{
...
declare function tk:closestTopKByElement($document as node()*, $oneway as node(), $topK)
{
 tk:closestTopKByName($document,$oneway/@name,$topK)
};

declare function tk:closestTopKWrtNameByKeywords($document as node()*, $name as xs:string, 
...
declare function tk:closestTopKWrtElementByKeywords($document as node()*, $oneway as node(), 
                  $keywordCollection as xs:string*,$topK as xs:integer)
{
  tk:closestTopKWrtNameByKeywords($document,$oneway/@name,$keywordCollection,$topK)
};

declare function tk:closestDistanceByName($document as node()*,
                           $name as xs:string, 
...
declare function tk:closestDistanceByElement($document as node()*, 
                           $oneway as node(), 
                           $closetDistance as xs:double)
{
...
declare function tk:closestDistanceWrtNameByKeywords($document as node()*, 
                  $name as xs:string, 
...
declare function tk:closestDistanceWrtElementByKeywords($document as node()*, 
                  $oneway as node(), 
                  $keywordCollection as xs:string*,
                  $closetDistance as xs:double)
{
...
declare function tk:closestElementByCollection($name as xs:string,
             $collectionElements as node()*)
{
 (: Importante: pasamos una collecion y tiene que devolcer el elemento más cercano al 
 elemento representado por $name:)
 (: Para ello debemos obtener el oneway de name, y para cada oneway de collection 
 debemos obtener la distancia con respecto al oneway de name.
 Una vez obtenida, ordeanamos y nos quedamos con el primero tirando de la función
 subsequcnce para obtener el primero :)
 
 $name
 
};

declare function tk:getMinDistTopK($candidates as node())
{
 xs:double(data($candidates/@mindist))
};

declare function tk:getMaxDistTopK($candidates as node())
{
 xs:double(data($candidates/@maxdist))
};

declare function tk:getNumberObjectsTopK($candidates as node())
{
 xs:integer(data($candidates/@objects))
};

declare function tk:addCandidate($candidates as node(), $oneway as node(), $mindist as xs:double, 
                                 $numberObjects as xs:integer, $flag as xs:boolean)
{
  let $document := 
    <doc>
    {
  ...
  return 
      ...
      objects = "{$numberObjects}">
       {
         if ($flag) then remove($doc,$numberObjects+1) 
         else $doc
       }
      </candidates>
};
...
declare function tk:addOneWayTopK($candidates as node(), $oneway as node(), $topK as xs:integer)
{
...
declare function tk:addKeywordDistance($candidates as node(), $oneway as node(), 
...
declare function tk:addOneWayDistance($candidates as node(), $oneway as node(), $closetDistance as xs:double)
{
