...
declare function osm:addTagMode($oneway as node(), $kValueToAdd as xs:string, $vValueToAdd as xs:double)
{
  let $name := $oneway/@name
    return
    <oneway name = "{$name}">
...
{
 geo:length(osm_gml:_osm2GmlLine($oneway))
};

declare function osm:getArea($oneway as node())
{
 geo:area(osm_gml:_osm2GmlPolygon($oneway)) 
};
