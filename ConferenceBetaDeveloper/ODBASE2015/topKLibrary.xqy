...
if ($document/node) then 
  tk:getLayer_each_node_k($document/node,$root,$mbr,$keywordCollection,
                    $candidates,$flag,$closetDistance,$topK)
else 
   if ($document/leaf) then 
     tk:getLayer_each_leaf_k($document/leaf,$root,$mbr,$keywordCollection,
                    $candidates,$flag,$closetDistance,$topK)
   else
     tk:getLayer_each_mbr_k($document/mbr,$root,$mbr,$keywordCollection,
                    $candidates,$flag,$closetDistance,$topK)
};
...
{  
 if (empty($document)) then 
  ...
  return 
    if ((if ($flag) then (tk:getNumberObjectsTopK($candidates) < $topK) else false())
    or  (if ($flag) then (tk:minDistanceMBR(xs:float($node/@x),xs:float($node/@y),
                          xs:float($node/@z),xs:float($node/@t),
                          xs:float($mbr/@x),xs:float($mbr/@y),xs:float($mbr/@z),
                          xs:float($mbr/@t),$distanceMBR)) else 
                         (tk:minDistanceMBR(xs:float($node/@x),xs:float($node/@y),
                         xs:float($node/@z),xs:float($node/@t),
                         xs:float($mbr/@x),xs:float($mbr/@y),xs:float($mbr/@z),
                         xs:float($mbr/@t),$closetDistance)))
                       )
    then                      
                 ...
                 if ($keywordCollection = " ")
                    then tk:getLayer_each_mbr_k(tail($document),$root,$mbr," ",
                                        ...
                                        $flag,$closetDistance,$topK) 
                                         ...
                                         tk:addKeywordTopK($candidates, $oneway,
                                         $keywordCollection,$topK)
                                         ,$flag,$closetDistance,$topK)
              else 
                 if ($keywordCollection = " ")
                    then tk:getLayer_each_mbr_k(tail($document),$root,$mbr," ",
...
declare function tk:closestElementByCollection($onewayRef as node(),
