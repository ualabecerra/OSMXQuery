...
{
  file:write($layer,rt:ways_rtree(rt:ways($document) union rt:nodes($document),(),$k))
};

declare function rt:getLayer_mbr($document,$mbr)
{
if ($document/node) then  for $node in $document/node 
                              
                                where 
                                rt:overlap(xs:float($node/@x),xs:float($node/@y),xs:float($node/@z),xs:float($node/@t),xs:float($mbr/@x),xs:float($mbr/@y),xs:float($mbr/@z),xs:float($mbr/@t))
                                 
                                return rt:getLayer_mbr($node,$mbr)
                                else  
                                for $leaf in $document/leaf
                                where rt:overlap(xs:float($leaf/@x),xs:float($leaf/@y),xs:float($leaf/@z),xs:float($leaf/@t),xs:float($mbr/@x),xs:float($mbr/@y),xs:float($mbr/@z),xs:float($mbr/@t))
                                
                                
                                return for $way in $leaf/mbr 
                                where rt:overlap(xs:float($way/@x),xs:float($way/@y),xs:float($way/@z),xs:float($way/@t),xs:float($mbr/@x),xs:float($mbr/@y),xs:float($mbr/@z),xs:float($mbr/@t))
                                 
                                return
                                if ($way/mbr/way) then <oneway name="{($way/mbr/way)[1]/tag[@k="name"]/@v}"> {$way/mbr/*} </oneway> else <oneway  name="node"> {$way/mbr/*} </oneway> 
                  
                                
  
};

declare function rt:getLayerByName($document,$name)
{
  for $mbr in $document//mbr[*/tag/@v=$name]
  return  
   rt:getLayer_mbr($document,$mbr)
};

declare function rt:getLayerByOneWay($document,$oneway as node())
{
  let $stringName := $oneway/@name
  return rt:getLayerByName($document,$stringName)
};

declare function rt:overlap($x,$y,$z,$t,$a,$b,$c,$d)
{
