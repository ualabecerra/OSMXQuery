...
declare function rt:getLayer_mbr($document,$mbr,$distance)
{
    ...
    {  $document/rtree/node }
    return
     rt:getLayer_mbr_node($doc,$document,$mbr,$distance)
...
if ($document/node) then  for $node in $document/node
  ...
  (:MODIFICACION:) (: Aquí es donde tiene que seleccionar los k candidatos :)
...
(:cambiar y poner distancia máxima:)
