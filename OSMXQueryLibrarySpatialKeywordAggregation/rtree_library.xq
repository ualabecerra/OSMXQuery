...
declare function rt:getLayer_mbr($document,$mbr,$distance,$flag)
{
    let $doc := document
    {  $document/rtree/node }
    return
    rt:getLayer_mbr_node($doc,$document,$mbr,$distance,$flag)
...
if ($document/node) then  for $node in $document/node
                                ...
                                where
                                rt:overlap(xs:float($node/@x),xs:float($node/@y),xs:float($node/@z),xs:float($node/@t),
                                xs:float($mbr/@x),xs:float($mbr/@y),xs:float($mbr/@z),xs:float($mbr/@t),$distance)
                                
                                return  rt:getLayer_mbr_node($node,$root,$mbr,$distance)
                                else  
                                for $leaf in $document/leaf
                                where rt:overlap(xs:float($leaf/@x),xs:float($leaf/@y),xs:float($leaf/@z),xs:float($leaf/@t),
                                xs:float($mbr/@x),xs:float($mbr/@y),xs:float($mbr/@z),xs:float($mbr/@t),$distance)
                                
                                
                                return for $way in $leaf/mbr 
                                where rt:overlap(xs:float($way/@x),xs:float($way/@y),xs:float($way/@z),xs:float($way/@t),
                                xs:float($mbr/@x),xs:float($mbr/@y),xs:float($mbr/@z),xs:float($mbr/@t),$distance)
...
declare function rt:getLayer_mbr_node($document,$root,$mbr,$distance,$flag)
{
 if ($flag=0) then
  if ($document/node) then  for $node in $document/node
                                ...
                                return  rt:getLayer_mbr_node($node,$root,$mbr,$distance,$flag)
                                else  
                                for $leaf in $document/leaf
                                where rt:overlap(xs:float($leaf/@x),xs:float($leaf/@y),xs:float($leaf/@z),xs:float($leaf/@t),
                                xs:float($mbr/@x),xs:float($mbr/@y),xs:float($mbr/@z),xs:float($mbr/@t),$distance)
...
{
 if (rt:overlap_old($x,$y,$z,$t,$a,$b,$c,$d)) then true()
 else
...
declare function rt:getLayerByName($document,$name,$distance)
{
  ...
  return 
    rt:getLayer_mbr($document,$mbr,$distance,1)
};

declare function rt:getLayerByElement($document as node()*,$oneway as node(), $distance)
{
  if ($oneway/way)
  then  rt:getLayerByName($document,$oneway/@name,$distance) 
  else
         let $node := $oneway/*[1]
         let $lat := $node/@lat, $lon := $node/@lon
         return rt:getLayer_mbr($document, 
         <mbr x = '{$lon}' y = '{$lat}' z = '{$lon}' t = '{$lat}'/>,$distance,0)
(:   rt:getLayerByName($document,$oneway/@name,$distance) :)
};

declare function rt:getElementsByKeyword($document as node()*, $string as xs:string)
{
