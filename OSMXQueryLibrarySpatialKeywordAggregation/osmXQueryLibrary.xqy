...
declare function osm:intersectionQuery($onewayResult1 as node()*, 
...
declare function osm:searchKeywordCollection($oneway as node(), $keywordCollection as xs:string*)
{
  if (some $value in 
  (distinct-values(
  for $keyword in $keywordCollection
    return osm:searchKeyword($oneway,$keyword))) satisfies ($value = true()))
    then true()
    else false()
};

declare function osm:searchKeyword($oneway as node(), $keyword as xs:string)
{
...
declare function osm:searchTag($oneway as node(), $kValue as xs:string, $vValue)
{
